...
// Misma paleta que el resto del panel de IND (negro/crema), no la azul de MANDI.
const C = {
  bg: '#0A0A0A', surface2: '#111111', border: '#1F1F1F', border2: '#2A2A2A',
...
const sTextarea = {
  ...
  color: C.cream, fontSize: 11, padding: '6px 8px', resize: 'vertical', outline: 'none',
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          background: guardando ? C.bg : 'rgba(245,158,11,.12)',
      ...
      {cargando ? (
        <div style={{ fontSize: 10, color: C.creamDim, marginTop: 8 }}>Cargando notas...</div>
      ) : notas.length === 0 ? (
        <div style={{ fontSize: 10, color: C.creamFaint, marginTop: 8 }}>Sin notas todavía.</div>
      ) : (
          ...
          {notas.map(nota => {
            ...
            return (
              <div key={nota.id} style={{
                border: `1px solid ${est ? est.borde : C.border}`, borderRadius: 7, overflow: 'hidden',
                background: est ? est.fondo : C.surface2,
              }}>
                <div
                  onClick={() => alternar(nota.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', cursor: 'pointer' }}>
                  <span style={{ fontSize: 8, color: C.creamDim, flexShrink: 0 }}>{abierta ? '▼' : '▶'}</span>
                  <span style={{ fontSize: 10, color: C.creamDim, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>
                  ...
                  {!abierta && (
                    <span style={{ fontSize: 11, color: est ? est.color : C.creamDim, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                ...
                {abierta && (
                  <div style={{ padding: '7px 8px 8px', borderTop: `1px solid ${C.border}` }}>
                    {editando ? (
                      <>
                        ...
                        <div style={{ display: 'flex', gap: 5, marginTop: 5 }}>
                          ...
                          <button onClick={() => setEditandoId(null)}
                            style={{ flex: 1, padding: 5, background: 'rgba(255,255,255,.04)', border: `1px solid ${C.border2}`, color: C.creamDim, borderRadius: 6, fontSize: 10, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
                        </div>
                      </>
                    ) : (
                      <>
                        ...
                        <div style={{ fontSize: 11, color: est ? est.color : C.cream, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                          {est ? `${est.marca} ` : ''}{nota.texto}
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6 }}>
                          <button onClick={() => copiarNota(nota.id, nota.texto)}
                            style={{ background: 'none', border: 'none', padding: 0, color: copiadoId === nota.id ? VERDE : C.creamDim, fontSize: 10, fontWeight: copiadoId === nota.id ? 700 : 400, cursor: 'pointer', fontFamily: 'inherit' }}>
                            {copiadoId === nota.id ? '✅ Copiado' : '📋 Copiar'}
                          </button>
                          <button onClick={() => { setEditandoId(nota.id); setEditTexto(nota.texto) }}
                            style={{ background: 'none', border: 'none', padding: 0, color: C.creamDim, fontSize: 10, cursor: 'pointer', fontFamily: 'inherit' }}>✏️ Editar</button>
                          {confirmarBorrar === nota.id ? (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                              ...
                              <button onClick={() => setConfirmarBorrar(null)}
                                style={{ background: C.border2, border: 'none', color: '#fff', fontSize: 10, padding: '2px 8px', borderRadius: 5, cursor: 'pointer', fontFamily: 'inherit' }}>No</button>
                            </span>
                          ) : (
                            <button onClick={() => setConfirmarBorrar(nota.id)}
                              style={{ background: 'none', border: 'none', padding: 0, color: C.creamFaint, fontSize: 10, cursor: 'pointer', fontFamily: 'inherit' }}>✕ Borrar</button>
                          )}
                        </div>
                      </>
                    )}
