...
// ── Capas de gestión (diseño 2026-09-22: docs/superpowers/specs/2026-09-22-gestion-chats-ind-design.md) ──
// Bandeja 🔴🟢⚫ (de quién es el turno) · temperatura AUTOMÁTICA (lib/temperatura.js,
// cuánto hace que habló el cliente) · etapa 💬💳🛒🔁 · 📌 le debemos · 🤫 · tipo de contacto.
// Un mensaje del cliente SOLO mueve la bandeja: nunca borra etapa, 📌, 🤫 ni tipo.
const VENTANA_MS = 24 * 60 * 60 * 1000

// Al RESPONDER una persona, la bandeja pasa a 'atendido'. Siempre: SOPORTE ya no es
// bandeja (la derivación de la IA es 🔴 + 📌 🎧, ver lib/gestion.js).
...
const BANDEJAS = [
  ...
  { key:'archivado', icon:'⚫', label:'Archivar',  color:C.creamDim },
]
...
const Separador = () => <span style={{ width:1, alignSelf:'stretch', background:C.border2, margin:'2px 2px', flexShrink:0 }} />
...
export default function App() {
  ...
  // adjunto en vez de nota de voz es la pantalla mintiendo.
  ...
  useEffect(() => {
    ...
    if (!anclaScrollRef.current && (autoScroll.current || hadNewMsg)) endRef.current?.scrollIntoView({ behavior: hadNewMsg ? 'smooth' : 'instant' })
  }, [active, convs])
  ...
  // anteriores, y así hasta el primer mensaje del cliente. Antes no había forma
  // de pedir más: lo anterior quedaba en la base sin que nadie lo mirara.
  const cargandoMasRef = useRef(false)
  const sinMasRef      = useRef({})   // teléfono → ya no queda historial
  ...
  const cargarMasHistorial = useCallback(async () => {
    ...
    if (!tel || cargandoMasRef.current || sinMasRef.current[tel]) return
    const actuales = hilosRef.current[tel] || []
    ...
    setCargandoMas(true)
    // ⚠️ El ancla se toma ANTES de pedir: después del render la altura ya cambió
    // y la pantalla habría saltado.
    const el = msgsRef.current
    anclaScrollRef.current = el ? { alto: el.scrollHeight, top: el.scrollTop } : null

    try {
      const previos = await fetchHilo(tel, 800, masViejo)
      ...
      if (fusion.length <= actuales.length) {
        sinMasRef.current[tel] = true
      } else {
        hilosRef.current[tel] = fusion
  ...
  // Una notificación del navegador por chat y por ventana. El permiso lo pide el
  // botón 🔔 (PushToggle) dentro de su click: Chrome silencia pedidos sin gesto.
  useEffect(() => {
    const now = Date.now()
  ...
  // ── Gestión del chat (diseño 2026-09-22) ──────────────────────────────
  // Todo lo que las reglas puras necesitan de un chat. La temperatura NO viene de la
  // base: se calcula con `ahora`, que avanza cada minuto para que los chips se muevan.
  ...
  // filtros COMBINABLES (bandeja × temperatura × etapa × 📌 × 🎧 × ⏰ × interno), y por canal
  ...
  // (el poll puede traer una lectura vieja del edge; lib/overrides.js)
  ...
  const cambiarDeuda = (telefono) => {
    ...
    const nota = window.prompt('¿Qué le debemos al cliente? (boceto, guía, precio, respuesta…)', '')
  ...
  const agregarAdjuntos = async (entrantes) => {
    ...
    if (plan.tipo === 'audio') {
      // El audio NO pasa por `toJpeg` ni genera miniatura: se muestra su nombre y
      // el aviso de formato.
  ...
  const handleSendImage = async () => {
    ...
    try {
      ...
      if (isAudio) {
        ...
        const result = await sendAudio(telefono, nombre, archivos[0].file, {
          onProgreso: (pr) => setImgProgress(Math.round(pr * 100) / 100),
        })
        allOk = result.ok
        ...
        const result = await sendDocumento(telefono, nombre, archivos[0].file, caption)
        allOk = result.ok
        if (!result.ok) sendErr = result.error || ''
  ...
  const handleQuickReply = async (reply, onProgress) => {
    ...
    // WhatsApp entrega cada uno como un mensaje aparte, así que este orden es el
    // que ve el cliente: texto que presenta, foto que muestra, voz que cierra.
    // Ver lib/adjuntos-respuesta.js.
                ...
                <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:10, fontWeight:700, color:C.creamDim, display:'flex', alignItems:'center', gap:3, marginTop:1 }}>
                      ...
                      {`En vivo · ${totalPendientes} pendiente${totalPendientes === 1 ? '' : 's'}`}
                    </div>
                  </div>
                </div>
          ...
          {activeConv ? (
            ...
            <div className="chat-col" style={{ position:'relative' }}
              ...
              <div className="chat-header" style={{ padding:'8px 10px', background:C.surface, borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', flexWrap:'wrap', flexShrink:0, gap:6 }}>
                ...
                <div className="chat-actions" style={{ display:'flex', alignItems:'center', gap:4, flexWrap:'wrap', flex:1, justifyContent:'flex-end' }}>
                  ...
                  {Object.entries(ETAPAS).map(([key, e]) => {
                    ...
                    return (
                      <BotonEje key={key} on={on} color={e.color} icon={e.icon} label={e.label}
                        ...
                        onClick={() => cambiarEtapa(activeConv.telefono, key)} />
                    )
                  })}
                  <Separador />
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <BotonEje on={Boolean(ct.deudaAt)} color={ct.deudaAt ? '#fbbf24' : C.creamDim}
                          icon={ct.deudaPor === 'ia' ? '📌🎧' : '📌'}
                        ...
                        <BotonEje on={Boolean(ct.sinAutomaticos)} color={C.creamDim} icon="🤫" label="Sin automáticos"
                          title={ct.sinAutomaticos ? 'No le llegan mensajes automáticos — clic para permitirlos' : 'Que el sistema no le escriba solo (prometió algo, hay una queja…)'}
                          onClick={() => cambiarSinAutomaticos(activeConv.telefono)} />
                        <BotonEje on={ct.tipoContacto === 'interno'} color={C.creamDim} icon="🏷️" label="Interno"
              ...
              <div ref={msgsRef} className="msgs-scroll" onScroll={handleMsgsScroll} style={{ background:`radial-gradient(ellipse at 20% 10%, rgba(244,241,236,.015) 0%, transparent 60%)` }}>
                {/* ⚠️ Que se VEA que está trayendo historial. Sin aviso, subir en
                    un chat largo se siente como que la pantalla se trabó. */}
                {cargandoMas && (
                  <div style={{ textAlign:'center', padding:'10px 0', fontSize:11, color:C.creamFaint }}>
                      ...
                      {imgFiles.map((item, i) => (
                        <div key={i} style={{ position:'relative' }}>
                          {isAudio
                            ...
                            // como "algo falló" cuando está todo bien.
                      ...
                      <span style={{ fontSize:10, color:C.creamFaint }}>
                        {imgUploading
                          ...
                          : imgResult
                            ...
                            // Se avisa que el texto viaja PEGADO al archivo. Sin el aviso, el
                            // vendedor no sabe si tiene que mandarlo aparte.
