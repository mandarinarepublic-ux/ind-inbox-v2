...
// ── Notificación de pago dLocal (IND STORE) ─────────────────────────────────
// Portada desde wa-inbox-next (MANDI) el 15-ago-2026 por decisión explícita
// del dueño: ver lib/dlocal.js. dLocal llama aquí (notification_url) con un
// payment_id cuando cambia el estado del pago — el mismo link generado en
// app/api/linkpago/route.js contra la cuenta dLocal COMPARTIDA con MANDI.
// Consultamos el detalle y avisamos por WhatsApp (plantilla "confirmacionpago_dlocalgo")
// al CLIENTE y al SOPORTE, igual que en MANDI.
//
// ⚠️ Esta plantilla se pidió aprobar en la WABA de MANDI. NO se pudo confirmar
// en este port que también esté aprobada en la WABA de IND (las plantillas de
// IND son, a la fecha, una pregunta abierta — ver GET /api/plantillas). Si no
// existe, Meta la va a rechazar y quedará solo el log de error; el pago igual
// se registra y la nota interna (verde/roja) igual se guarda, porque
// guardarNotaPago corre ANTES y está envuelta aparte.
//
...
//
// Apuntar en env: DLOCAL_NOTIFY_URL = https://ind-inbox-v2.vercel.app/api/pago-dlocal?k=<secreto>
//
...
const GRAPH_URL      = `${GRAPH}/${META_PHONE_ID}/messages`
...
// Plantilla confirmacionpago_dlocalgo (6 parámetros), misma lógica que MANDI.
