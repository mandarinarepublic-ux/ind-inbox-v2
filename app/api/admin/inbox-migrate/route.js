// app/api/admin/inbox-migrate/route.js — BACKFILL TEMPORAL Sheets → Supabase (inbox, cuenta=IND).
...
// Fecha del Sheet → ISO. IND mezcla ISO (webhook nuevo) y "07/jul/2026 11:55" (Make legacy).
function fechaISO(v) {
  const d = parseDate(v)
  return isNaN(d.getTime()) ? new Date('2020-01-01T00:00:00Z').toISOString() : d.toISOString()
}
...
export async function GET(req) {
  ...
  try {
    ...
    if (tabla === 'respuestas') {
      ...
      const rs = rows.filter(r => r[0] && r[1] && r[0] !== 'ID').map(mapRespuestaRow)
      const filas = rs.map(r => ({
        cuenta: CUENTA, id: String(r.id), texto: r.text || '',
