...
import { toMensaje, pintable, paginarLimite } from '../lib/inbox-supabase.js'
...
// AHORA por toMensaje. Sin esto, pintable las sigue descartando y la persona
// queda invisible aunque la ingestión ya esté arreglada — eso es justo lo que
// pasó: 114 personas invisibles en IND pese al deploy.
...
test('toMensaje deriva el order de una fila vieja (texto vacio) usando raw', () => {
  ...
  assert.ok(pintable(m), 'tiene que pasar el filtro de pintable')
})
...
// 90% de la factura de Vercel en IND) dejo de pedir `raw` por peso (539 kB
// por ventana de 3.000 mensajes). Una fila de ESA consulta llega sin la
// clave `raw` (no `raw: null`, sino ausente del objeto) — este test es el
// que tiene que cazarlo si alguien vuelve a optimizar la consulta manana y
// esconde gente otra vez.
test('toMensaje: un order de la consulta SIN raw (polling) sigue pasando el filtro', () => {
  ...
  assert.ok(pintable(m), 'una fila de polling sin raw NO puede quedar invisible')
})
