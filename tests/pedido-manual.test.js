...
test('la url del pedido lleva embed y va al dominio del CRM', () => {
  const url = new URL(urlVerPedido('IND-AND-5563'))
  assert.strictEqual(url.origin, 'https://crm.apps.mandarinaec.com')
  assert.strictEqual(url.pathname, '/dashboard/pedido/IND-AND-5563')
  assert.strictEqual(url.searchParams.get('embed'), '1')
})
...
test('la url se arma en el MISMO dominio que el formulario', () => {
  ...
  // con `MANDARINACRM_URL`, que hoy cae por defecto en el dominio de Vercel
  // (`mandarina-pro-sales.vercel.app`) — ahí la cookie NO viaja y el panel
  // mostraría un login en vez del pedido. Por eso no se usa esa url y se arma
  // de cero con el número.
  assert.strictEqual(
    new URL(urlVerPedido('IND-AND-5563')).origin,
...
test('el número de pedido va escapado', () => {
  ...
  const url = new URL(urlVerPedido('IND 2026/0412?x=1'))
  assert.strictEqual(url.pathname, '/dashboard/pedido/IND%202026%2F0412%3Fx%3D1')
  assert.strictEqual(url.searchParams.get('embed'), '1')
...
test('saca el número de pedido de la url de la nota', () => {
  const nota = textoNotaPedido(avisoDelCrm({
    pedidoId: 'IND-2026-0412', montoTotal: 78.9,
    url: 'https://crm.apps.mandarinaec.com/dashboard/pedido/IND-2026-0412',
  }))
  assert.strictEqual(pedidoIdDeNota(nota), 'IND-2026-0412')
})
...
test('el número gana aunque la nota traiga la url del dominio VIEJO', () => {
  ...
  const nota = '📦 Pedido IND-2026-0500 · $12\nhttps://mandarina-pro-sales.vercel.app/dashboard/pedido/IND-2026-0500'
  assert.strictEqual(pedidoIdDeNota(nota), 'IND-2026-0500')
  assert.strictEqual(
...
test('el número escapado en la url vuelve a su forma original', () => {
  assert.strictEqual(pedidoIdDeNota('mira https://crm.apps.mandarinaec.com/dashboard/pedido/IND%202026'), 'IND 2026')
})
...
test('una nota común no inventa un pedido', () => {
  ...
  assert.strictEqual(pedidoIdDeNota('https://indstore.ec/products/algo'), '')
...
test('acepta la hoja que manda el CRM', () => {
  assert.deepStrictEqual(
    hojaDelCrm({ pedidoId: 'IND-2026-0412', imagen: JPG }),
    { pedidoId: 'IND-2026-0412', imagen: JPG }
  )
})
...
test('RECHAZA una hoja de otro origen', () => {
  ...
  assert.strictEqual(leerHojaPedido({
    origin: 'https://evil.com',
    data: { tipo: 'hoja-pedido', pedidoId: 'IND-1', imagen: JPG },
  }), null)
...
test('RECHAZA lo que no sea un JPG en base64', () => {
  ...
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: 'https://evil.com/foto.jpg' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: 'data:image/svg+xml;base64,PHN2Zz4=' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: 'data:text/html;base64,PGI+' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: 'data:image/jpeg;base64,no válido!' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: '' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1' }), null)
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: { toString: () => JPG } }), null)
})

test('RECHAZA otro tipo de mensaje y uno sin pedido', () => {
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: JPG, tipo: 'otra-cosa' }), null)
  ...
  assert.strictEqual(hojaDelCrm({ pedidoId: 'I1', imagen: JPG, tipo: 'pedido-creado' }), null)
  assert.strictEqual(leerAvisoPedido({
    origin: 'https://crm.apps.mandarinaec.com',
    data: { tipo: 'hoja-pedido', pedidoId: 'I1', imagen: JPG },
  }), null)
})
...
test('la nota del pedido sale IDÉNTICA a la del camino con IA', () => {
  ...
  const respuestasDelAgente = [
    ...
    { pedidoId: 'IND-2026-0413', montoTotal: 120,     url: 'https://crm.apps.mandarinaec.com/dashboard/pedido/IND-2026-0413' },
    { pedidoId: 'IND-2026-0414', montoTotal: '45.00', url: 'https://crm.apps.mandarinaec.com/dashboard/pedido/IND-2026-0414' },
...
test('la ✕ del cajón y tocar fuera también preguntan', () => {
  // Los dos cierran el panel del celular y con él el formulario, sin cambiar de
  // chat. Se expresan igual que cerrar el chat — `destino` en null — para que
  // pasen por el mismo guard.
  //
  // Ojo con la diferencia frente a MANDI: allá el 👋 (AUTOMATIZACIONES) también
  // pasa por acá porque desmonta el panel derecho. En IND las vistas CONTACTOS y
  // AUTOS solo ESCONDEN el chat con `display:none`, así que el formulario
  // sobrevive intacto y preguntar ahí sería puro ruido.
