...
export const DEFAULTS = {
  ...
  // 🔄 REACTIVACIÓN por etapa (diseño 2026-09-22 §5, lib/reactivacion.js): le escribe
  // solo a un cliente en 💬/💳 que se quedó callado después de NUESTRO mensaje, a
  // las `horas` de su último mensaje (un texto por toque y por etapa), nunca de
  // 22:00 a 08:00, nunca con 📌, 🤫, pedido o contacto interno. APAGADA: la prende
  // una persona en la pestaña AUTOS. Se guarda el bloque COMPLETO (merge de un nivel).
  reactivacion: {
    activo: false,
    horas: [3, 12, 20],
    textos: {
  ...
  // FLUJOS (puerto desde MANDI, 15-sep-2026). INTERRUPTOR GENERAL de todos los
  // flujos publicados: apagado, el webhook ni lee la tabla y el cron no manda nada.
  // Arranca PRENDIDO: un flujo solo corre si alguien lo publicó a mano, así que
  // esto no enciende nada por su cuenta. Un solo nivel (merge() es de UN nivel).
  flujos: { activo: true },
}
