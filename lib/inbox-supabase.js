...
export async function registrarContactoEntranteSupabase(telefono, nombre, waId) {
  ...
  // Regresar a PENDIENTE salvo que esté ARCHIVADO o ya PENDIENTE (ahorra writes).
  const estadoActual = String(exist.estado || '').trim().toUpperCase()
  if (estadoActual !== 'ARCHIVADO' && estadoActual !== 'PENDIENTE') patch.estado = 'PENDIENTE'
