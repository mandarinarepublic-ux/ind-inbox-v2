...
/**
 * Avisa que Meta mandó un evento a nivel CUENTA sobre uno de nuestros números
 * (webhook `account_update`: ACCOUNT_OFFBOARDED, DISABLED_UPDATE, restricciones…).
 *
 * A diferencia de `reportarFalloCapi` —una señal de pauta que se cae— esto es la
 * base del negocio: si el número se offboardea, NO entra ni sale un solo mensaje
 * por ese canal, se cayó la tienda entera de ese número. El 10-ago-2026 el 9804
 * se offboardeó a las 15:24 y el equipo se enteró recién por las ventas perdidas,
 * porque este evento se guardaba en silencio en `webhook_eventos`. Esta alerta
 * cierra ese hueco: el mismo chat de siempre, pero con el texto más fuerte.
 *
 * Reusa el silenciador por (cuenta, código) de `reportarFalloCapi`: Meta reenvía
 * el mismo account_update varias veces y no queremos repetir el aviso. El código
 * sintético incluye el evento y el WABA para no tapar dos alertas distintas.
 *
 * No lanza nunca: un fallo al avisar no puede tumbar el webhook (Meta necesita su 200).
 *
 * @param {object} a
 * @param {string} a.evento   valor de change.value.event (ACCOUNT_OFFBOARDED, …)
 * @param {string} [a.canal]  etiqueta legible del número (p.ej. '9804')
 * @param {string} [a.wabaId] whatsapp_business_account_id (entry[0].id)
 */
export async function reportarAlertaCuenta({ evento, canal, wabaId }) {
  try {
    const crm = getCrm()
    if (!crm) return { omitido: 'sin_supabase' }

    const ev = String(evento || 'DESCONOCIDO')
    const codigo = `cuenta:${ev}:${wabaId || ''}`
    if (await yaAvisado(crm, codigo)) return { omitido: 'ya_avisado' }

    const canalTxt = canal || `WABA ${wabaId || '—'}`
    // Eventos que dejan el número MUERTO (no entra ni sale nada). El resto de los
    // account_update (cambios de nombre, de foto) no son urgencias.
    const critico = /OFFBOARD|DISABLED|BAN|RESTRICT|DELETE/i.test(ev)

    await crm.from('eventos_sistema').insert({
      fuente: 'meta',
      nivel: 'error',
      mensaje: `Meta account_update ${ev} en ${CUENTA} · canal ${canalTxt}`,
      pedido_id: null,
      detalle: {
        origen: 'inbox',
        cuenta: CUENTA,
        evento: `account_update:${ev}`,
        // `codigo` es lo que mira el silenciador (yaAvisado). Con el evento + WABA
        // adentro, dos alertas distintas no se tapan entre sí.
        codigo,
        canal: canalTxt,
        waba_id: wabaId || null,
        critico,
    ...
    await avisarTelegram(
      `🛑 *NÚMERO CAÍDO — no es una venta*\n` +
      `Meta dio de baja el número *${canalTxt}* de *${CUENTA}*.\n\n` +
      `Evento: *${ev}*  ·  WABA ${wabaId || '—'}\n\n` +
      (critico
        ? '⛔ Por este número NO entra ni sale ningún mensaje. Hay que re-onboardearlo en Meta Business Manager: re-vincular el WABA, re-registrar el número en la Cloud API y re-suscribir el webhook.'
        : 'ℹ️ Revisar el estado del número en Meta Business Manager.') +
      `\n\n_Los repetidos se silencian ${SILENCIO_MIN} min para no inundar el chat._`
    )

    return { reportado: true }
  } catch (e) {
    console.error('[eventos] reportarAlertaCuenta:', e.message)
    return { error: e.message }
  }
}
