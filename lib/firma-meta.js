...
export function evaluarFirmaMeta({ secreto, crudo, cabecera }) {
  ...
  // ☠️ EL ORDEN IMPORTA, y el original tapaba la única señal que queda cuando NO
  // hay secreto. Antes `sin-secreto` se evaluaba PRIMERO, así que mientras no
  // estuviera configurado TODO daba 'sin-secreto' — y alguien posteando sin
  // firma se veía exactamente igual que la operación normal.
  //
  // Meta manda SIEMPRE `x-hub-signature-256`. Que no venga, o que venga
  // deforme, se detecta sin comparar nada. Por eso se decide antes.
  if (!cabecera) return 'sin-cabecera'

  const m = String(cabecera).match(/^sha256=([a-f0-9]{64})$/i)
  if (!m) return 'formato-raro'

  // Recién acá hace falta el secreto: es para comparar de verdad.
...
/**
 * ¿Este veredicto merece despertar a alguien por Telegram?
 *
 * ⚠️ NO avisa por `sin-secreto`. Es nuestra propia falta de configuración, no un
 * ataque, y ocurre en CADA mensaje: IND recibe 2.684 webhooks al día, así que
 * sería un aviso cada 30 segundos. En dos horas nadie mira más esa alerta —
 * justo cuando llegue la de verdad. Una alarma que suena siempre está apagada.
 *
 * Tampoco avisa por un veredicto desconocido: si mañana se agrega uno nuevo, que
 * no empiece a sonar solo. Se decide a propósito si merece aviso.
 */
const AVISABLES = new Set(['sin-cabecera', 'formato-raro', 'NO-coincide'])
export function debeAvisar(veredicto) {
  return AVISABLES.has(String(veredicto || ''))
}
