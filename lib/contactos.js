...
export async function getModoIA(telefono) {
  ...
  // null = todos los numeros: el gate de la IA es del CLIENTE, no de la bandeja.
  const cs = await getContactos(null)
...
// Col H = IdVenta → se setea cuando se crea un pedido (PEDIDO MANUAL).
export async function updateIdVenta(telefono, idVenta) {
  return SB.updateIdVentaSupabase(telefono, idVenta)
}
// 💰 Venta en proceso: interruptor MANUAL del humano que atiende. Solo lo toca el
// botón del chat: ni los mensajes, ni los flujos, ni los pedidos lo cambian.
...
export async function derivarAPersona(telefono, nota) {
  return SB.derivarAPersonaSupabase(telefono, nota)
}
...
export async function marcarReactivacion(telefono, n) {
  return SB.marcarReactivacionSupabase(telefono, n)
}
...
// FLUJOS: un disparo por cliente cada 24 h (marca ANTES de enviar, con guardia).
...
// Anuncios vistos (inbox.anuncios), para el Disparador "Llega desde un anuncio".
// Siempre un arreglo: quien lo lee lo esparce.
