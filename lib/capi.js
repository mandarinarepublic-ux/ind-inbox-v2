...
/**
 * La etapa que el vendedor marca y que cuenta como venta en proceso.
 * ⚠️ IND se separó de MANDI acá el 22-sep-2026 (diseño de gestión de chats): la
 * temperatura dejó de ser manual y SOPORTE dejó de ser bandeja, así que la señal
 * humana pasó a ser 💳 Esperando pago o 🛒 Falta pedido. MANDI sigue con 🔥/SOPORTE.
 */
 ...
 *
 * Definición del negocio (1-ago-2026, ampliada el 8-ago, cambiada el 22-sep en
 * IND): el chat cuenta como venta en proceso si se cumple CUALQUIERA de estas:
 *
 *   - el vendedor lo marcó 💳 Esperando pago o 🛒 Falta pedido (antes: 🔥 CALIENTE
 *     o la bandeja SOPORTE)
 *   - el cliente MANDÓ UNA FOTO (agregada el 8-ago: es la señal más temprana)
...
export async function revisarVentaEnProceso(telefono) {
  try {
    ...
    const { data: conv } = await sb
      .from('conversaciones')
      .select('ctwa_clid, ctwa_captured_at, etapa')
      .eq('cuenta', CUENTA)
      .eq('telefono', telefono)
      .maybeSingle()
    ...
    // casi siempre es la prenda que quiere, o una captura del catálogo. Sin esto
    // ese cliente tenía que escribir 6 mensajes o esperar a que alguien lo marcara
    // caliente a mano, o sea que la señal le llegaba tarde a Meta o no le llegaba.
    //
    // Medido el 8-ago: 752 fotos entrantes en 30 días en IND. Es mucha señal que
    // no se estaba mandando.
    //
