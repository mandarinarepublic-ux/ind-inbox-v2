...
export function puertosDe(nodo) {
  ...
  if (tipo === 'mensaje') {
    ...
    // salida no tiene línea, la foto sigue por 'otra' como siempre (23-sep-2026).
 ...
 * 💬 Cotizando"), o '' si ninguno la fija. Solo 💬 y 💳: las demás las marca una
 * persona. Reemplaza a la temperatura de los nodos (diseño 2026-09-22): la
 * temperatura ahora es automática y un flujo ya no la toca.
 */
/**
 * 📌 "Le debemos al llegar acá": la última nota no vacía entre los nodos recorridos
 * (p. ej. el nodo "Ya se lo paso a nuestro diseñador" → 📌 "enviar boceto"). Una
 * promesa hecha por un automático también es una deuda nuestra (23-sep-2026).
 */
 ...
 *  - temperatura: la CALCULADA por tiempo desde el último mensaje del cliente
 *    (caliente · tibio · frio · dormido; lib/temperatura.js)
 *  - etapa: la etapa de la venta (cotizando · esperando_pago · falta_pedido · postventa)
...
export function puertoDeEntrante(nodo, { botonId = '', texto = '', esFoto = false, conFoto = false } = {}) {
  ...
  if (botones.length) {
    ...
    // Mandó una FOTO en vez de tocar un botón (caso real 22-sep: se perdía la mejor
    // señal de compra). Solo si el dueño conectó la salida 📸; si no, 'otra'.
...
export function decidirEntranteEnFlujo({ estado, flujo, entrante, ahora }) {
  ...
  const lineas = Array.isArray(flujo.grafo_vivo.lineas) ? flujo.grafo_vivo.lineas : []
  const conFoto = lineas.some((l) => l && l.de === nodo.id && l.puerto === 'foto')
