...
async function getJSON(path) {
  ...
  // ☠️ `fetch` NO lanza con 500: sin esto, un error del servidor se devuelve como
  // si fuera el dato. Costó real — `/api/hilo` estuvo devolviendo 500 y la carga
  // infinita lo leyó como "ya no hay más historial", que es lo PEOR que podía
  // concluir: se apagaba sola y en silencio hasta recargar la página.
  if (!res.ok) throw new Error(`${path} devolvió ${res.status}`)
  return res.json()
}
...
export async function fetchHilo(telefono, limite = 800, antesDe = '') {
  const antes = antesDe ? `&antesDe=${encodeURIComponent(antesDe)}` : ''
  try { return await getJSON(`/api/hilo?phone=${encodeURIComponent(telefono)}&limite=${limite}&canal=${CANAL_ACTIVO}${antes}`) }
  catch { return null }
}
...
// ¿Esta pestaña corre un bundle distinto del desplegado? Un deploy NO llega a una
// pestaña ya abierta. Vacío en cualquiera de los dos lados = no se sabe.
let BUILD_SERVIDOR = ''
...
export async function fetchInboxSync({ sinCache = false } = {}) {
  ...
  try {
    // ⚠️ La versión va por QUERY y vuelve en el CUERPO, no por cabecera. Con
    // `If-None-Match`/`ETag` el servidor logeó `cliente=NINGUNO` en todos los
    // polls: o el navegador no capturó el `ETag` de la respuesta, o una capa
    // intermedia lo quitó. El query y el cuerpo no se los toca nadie.
    //
    // `sinCache` = "acabo de cambiar algo, dame la verdad completa": ahí no se
    // manda la versión, para no depender del condicional justo en el momento en
    // que el vendedor está mirando si su clic surtió.
    const v = (!sinCache && ETAGS[canal]) ? `&v=${encodeURIComponent(ETAGS[canal])}` : ''
...
// 💰 Venta en proceso (manual, true/false). No depende de tener un pedido.
// Gestión (diseño 2026-09-22): etapa, 📌, 🤫 y tipo de contacto son de la PERSONA.
// Etiqueta CRM de cada chat (t9 → pedido). Falla en silencio: sin etiqueta el inbox sigue.
