...
export function parametrosReactivacion(r = {}) {
  const horas = (Array.isArray(r.horas) ? r.horas : HORAS_TOQUES)
    .map(Number).filter(n => Number.isFinite(n) && n >= 1 && n <= 23).map(n => Math.round(n))
  const unicas = [...new Set(horas)].sort((a, b) => a - b).slice(0, 3)
  const desde = acotar(r.hora_desde, 6, 12, HORA_DESDE)
  const hasta = Math.max(desde + 4, acotar(r.hora_hasta, 14, 22, HORA_HASTA))
  return {
    horas: unicas.length ? unicas : HORAS_TOQUES,
...
export function decidirReactivacion({ config, contacto: c, pedido = null, ahoraMs = Date.now() }) {
  ...
  const plantilla = Array.isArray(lista) ? String(lista[k] ?? '') : (TEXTOS_DEFECTO[etapa][k] || '')
