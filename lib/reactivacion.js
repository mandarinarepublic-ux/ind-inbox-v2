...
//   · el chat está 🟢 ATENDIDO y el último mensaje nuestro lo escribió una PERSONA
...
//   · no hubo otro automático en esta ventana (la encuesta), salvo nuestros toques (I3)
...
//
// 26-sep-2026: además de 💬/💳, con `incluir_sin_etapa` también le escribe a
// quien PREGUNTÓ y no quedó en ninguna etapa (la mayoría de los leads de pauta:
// 1.686 chats de anuncio en septiembre, 35 compraron). Esos van con los textos
// de la etapa 'pregunto'. Postventa y 🛒 falta pedido nunca (ya compraron).
// `solo_canales`: lista de phone_id por los que se permite escribir (p. ej. para
// no mandar por un número caído).
// Módulo PURO.
...
export const TEXTOS_DEFECTO = {
  pregunto: [
    '¡Hola {nombre}! 👋 Me preguntaste por nuestros hoodies personalizados 🖤 ¿Te ayudo a armar el tuyo? Mándame el diseño o la foto y tu talla, y te paso el precio final con envío incluido. Si es para un grupo (equipo, familia, promo), desde 6 prendas te hago precio especial 😉',
    '{nombre}, antes de que se cierre este chat 🙌 te dejo un regalo: con el código *IND10* tienes 10% de descuento si confirmas tu pedido hoy. Respóndeme "lo quiero" y lo dejamos listo 🖤',
    '',
  ],
  cotizando: [
...
export function decidirReactivacion({ config, contacto: c, pedido = null, ahoraMs = Date.now() }) {
  ...
  if (Array.isArray(r.solo_canales) && r.solo_canales.length && !r.solo_canales.includes(String(c.phoneId || ''))) return null
  const vigente = etapaVigente(c.etapa, c.etapaAt, pedido)
  // Sin etapa = solo preguntó. Con un pedido reciente ya salió arriba.
  const etapa = !vigente && r.incluir_sin_etapa ? 'pregunto' : vigente
  if (etapa !== 'cotizando' && etapa !== 'esperando_pago' && etapa !== 'pregunto') return null
  ...
  // Otro automático (la encuesta) en esta ventana que no fue nuestro toque → nada.
