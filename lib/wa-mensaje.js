...
// lo que produjo el bug de las fotos en MANDI (repo hermano): dos caminos que
// hay que acordarse de mantener iguales.
...
export function contenidoTipoEspecial(tipo, datos) {
  ...
  switch (tipo) {
    ...
    // pintable los deja pasar por ese lado. Ponerles la etiqueta genérica de
    ...
    case 'sticker':
      return ''
    // CUARTA VEZ que el filtro de "¿hay algo que pintar?" (pintable, en
    ...
    // llegó de verdad vacío (bug real, MANDI, repo hermano). SE ACABÓ LA
    // LISTA: la regla de ahora en adelante es "sin contenido nunca significa
    // invisible". Cualquier tipo sin caso explícito arriba —conocido con el
    // cuerpo vacío, o uno que WhatsApp todavía no inventó— cae acá y sale con
    // una etiqueta que nombra el tipo: visible Y diagnosticable. El próximo
    // tipo raro NO necesita un caso nuevo para dejar de esconder a la
    // persona; solo lo necesita si se quiere un texto más lindo que este
    // genérico.
    default:
...
//
// ⚠️ CUARTA VEZ que el filtro de "¿hay algo que pintar?" (lib/inbox-supabase.js)
// se come mensajes de verdad (antes: fotos sin caption, notas de voz, order/
// unsupported/system). Acá la regla que vale para SIEMPRE: "sin texto" NO
// significa "no pasó nada". Si Meta manda un tipo con contenido real (order) o
// un tipo donde igual pasó algo (reaction, edit, revoke, contacts,
// unsupported, system) o hasta un tipo que nadie previó todavía, este
// extractor le arma un `contenido` no vacío A PROPÓSITO — así el filtro lo
// deja pasar solo, sin tener que conocer estos tipos especiales. Ver
// contenidoTipoEspecial más arriba: el default de esa función es el que
// cierra el ciclo.
